...
@register_task("robowits/25_06-v0")
class WaterIntoMugMut6Env(WaterIntoMugEnv):
    @property
    def placement_groups(self) -> PlacementGroups | None:
        """Define placement groups for random initialization."""
        return (("pitcher", "water", "mug"), "heavy_large_object", "brick")
    ...
    def _add_custom_entities(self) -> None:
        ...
        _e = self._scene.scene.add_entity(
            ...
            surface=gs.surfaces.Smooth(color=(1.0, 0.6, 0.3), double_sided=True),
        )
        ...
        _e = self._scene.scene.add_entity(
            gs.morphs.Box(
                pos=(0.3, 0.10, 0.76 + 0.035), euler=(0, 0, 0), size=(0.22, 0.11, 0.07), fixed=False, collision=True
            ),
            material=gs.materials.Rigid(rho=1800.0, friction=0.9),
            surface=gs.surfaces.Rough(color=(0.7, 0.2, 0.1), double_sided=True),
        )
