...
@register_task("robowits/04_02-v0")
class PinchCardMut2Env(PinchCardEnv):
    @property
    def placement_groups(self) -> PlacementGroups | None:
        """Define placement groups for random initialization."""
        return ("bank card", "eraser", "eco bag", "soap bar")
    ...
    def _add_custom_entities(self) -> None:
        ...
        _e = self._scene.scene.add_entity(
            gs.morphs.Mesh(
                ...
                scale=0.326,
                pos=(0.70, 0.35, 0.7762),
                euler=(0.0, 0.0, 0.0),
                fixed=False,
                collision=True,
            ),
            material=gs.materials.Rigid(rho=200.0, friction=0.8),
            surface=gs.surfaces.Rough(color=(0.9, 0.9, 0.9), double_sided=True),
        )
