...
@register_task("robowits/08_05-v0")
class RoundDoughSheetMut5Env(RoundDoughSheetEnv):
    @property
    def placement_groups(self) -> PlacementGroups | None:
        """Define placement groups for random initialization."""
        return ("dough ball", "large flat board", "round cutter", "rolling pin")
    ...
    def _add_custom_entities(self) -> None:
        ...
        _e = self._scene.scene.add_entity(
            gs.morphs.Sphere(pos=(0.35, 0.00, 0.812), radius=0.04, fixed=False, collision=True),
        ...
        # Code Block: rolling pin
        _e = self._scene.scene.add_entity(
            gs.morphs.Cylinder(
                pos=(0.46, -0.12, 0.782), radius=0.02, height=0.32, euler=(0, 90, 0), fixed=False, collision=True
            ),
            material=gs.materials.Rigid(
                rho=700.0, friction=0.4, coup_friction=0.3, coup_softness=0.01, coup_restitution=0.0
            ),
            surface=gs.surfaces.Smooth(color=(0.76, 0.58, 0.35), double_sided=True),
        )
        self._entities["rolling pin"] = {"entity": _e}
        # Code Block: round cutter
        _e = self._scene.scene.add_entity(
            gs.morphs.Cylinder(
                pos=(0.66, -0.16, 0.788), radius=0.042, height=0.05, euler=(0, 0, 0), fixed=False, collision=True
            ),
            material=gs.materials.Rigid(
                rho=780.0, friction=0.3, coup_friction=0.3, coup_softness=0.01, coup_restitution=0.0
            ),
