...
@register_task("robowits/17_02-v0")
class BallOntoTowerMut2Env(BallOntoTowerEnv):
    @property
    def placement_groups(self) -> PlacementGroups | None:
        """Define placement groups for random initialization."""
        return (("tower base", "large ring", "medium ring"), "small ring", "ball", "funnel", "books", "rubber duck")
    ...
    def _add_custom_entities(self) -> None:
        ...
        _e = self._scene.scene.add_entity(
            gs.morphs.Mesh(
                ...
                scale=0.9,
                pos=(0.36, 0.3, 0.76 + 0.0611 * 0.9),
                euler=(0, 0, 0),
                fixed=False,
                collision=True,
            ),
            material=gs.materials.Rigid(rho=800.0, friction=0.9),
            surface=gs.surfaces.Smooth(double_sided=False),
        )
        self._entities["books"] = {"entity": _e}
        # Code Block: rubber duck
        _e = self._scene.scene.add_entity(
            gs.morphs.Sphere(pos=(0.34, -0.15, 0.76 + 0.03), euler=(0, 0, 0), radius=0.03, fixed=False, collision=True),
            material=gs.materials.Rigid(rho=500.0, friction=0.9),
            surface=gs.surfaces.Smooth(color=(1.0, 0.85, 0.1), double_sided=False),
        )
