...
@register_task("robowits/18_02-v0")
class CylinderThroughHoleMut2Env(CylinderThroughHoleEnv):
    @property
    def placement_groups(self) -> PlacementGroups | None:
        """Define placement groups for random initialization."""
        return ("cylindrical_peg", ("collection_zone", "hole_plate"), "hammer", "brick", "notebook")
    ...
    def _add_custom_entities(self) -> None:
        ...
        _e = self._scene.scene.add_entity(
            gs.morphs.Box(
                pos=(0.42, -0.2, 0.7925), euler=(0, 0, 0), size=(0.20, 0.095, 0.065), fixed=False, collision=True
            ),
            material=gs.materials.Rigid(rho=150.0, friction=0.9),
            surface=gs.surfaces.Rough(color=(0.8, 0.25, 0.2), double_sided=True),
        )
