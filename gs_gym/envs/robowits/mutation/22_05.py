...
@register_task("robowits/22_05-v0")
class StabilizeBottleMut5Env(StabilizeBottleEnv):
    @property
    def placement_groups(self) -> PlacementGroups | None:
        """Define placement groups for random initialization."""
        return (("sand_container", "dry_sand"), "tube", "bowl", "packing_peanuts")
    ...
    def _add_custom_entities(self) -> None:
        ...
        # Code Block: sand container
        _e = self._scene.scene.add_entity(
            gs.morphs.Mesh(
                coacd_options=coacd_options,
                file=get_asset_path("blender_kit/94134b02-c73e-4f2c-ad1d-a00a78160d98/obj.glb", pattern_is_dir=False),
                scale=1.0,
                pos=(0.68, -0.05, 0.8077),
                euler=(0, 0, 0),
                fixed=True,
                collision=True,
            ),
            material=gs.materials.Rigid(rho=700.0, friction=0.6),
            surface=gs.surfaces.Smooth(double_sided=True),
        )
        self._entities["sand container"] = {"entity": _e}
        # Code Block: tube
        _e = self._scene.scene.add_entity(
            gs.morphs.Mesh(
                coacd_options=coacd_options,
                ...
                scale=1.0,
                pos=(0.36, -0.05, 0.7744),
                euler=(0, 90, 0),
                fixed=True,
                collision=True,
            ),
            material=gs.materials.Rigid(rho=400.0, friction=0.6),
            surface=gs.surfaces.Smooth(double_sided=True),
        )
        ...
        _e = self._scene.scene.add_entity(
            gs.morphs.Cylinder(
                pos=(0.68, -0.05, 0.805), euler=(0, 0, 0), radius=0.018, height=0.04, fixed=False, collision=True
            ),
            material=gs.materials.MPM.Sand(rho=900.0, sampler="random", friction_angle=45),
            surface=gs.surfaces.Default(color=(0.9, 0.8, 0.3), vis_mode="particle"),
        )
        self._entities["dry sand"] = {"entity": _e}
