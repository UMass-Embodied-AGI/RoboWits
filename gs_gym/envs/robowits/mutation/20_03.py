...
@register_task("robowits/20_03-v0")
class BallIntoJarMut3Env(BallIntoJarEnv):
    @property
    def placement_groups(self) -> PlacementGroups | None:
        """Define placement groups for random initialization."""
        return ("glass jar", "foam ball", "pestle")
    ...
    def _add_custom_entities(self) -> None:
        ...
        _e = self._scene.scene.add_entity(
            gs.morphs.Mesh(
                ...
                scale=0.80,
                pos=(0.65, 0.00, 0.81334),
                euler=(0, 0, 0),
                fixed=True,
                collision=True,
            ),
            material=gs.materials.Rigid(rho=200.0, friction=0.8, coup_friction=0.2),
        ...
        _e = self._scene.scene.add_entity(
            gs.morphs.Cylinder(
                pos=(0.50, 0.10, 0.775), euler=(0, 90, 0), radius=0.015, height=0.18, fixed=False, collision=True
            ),
            material=gs.materials.Rigid(rho=600.0, friction=0.6, coup_friction=0.2),
            surface=gs.surfaces.Rough(color=(0.6, 0.45, 0.3), double_sided=True),
        )
