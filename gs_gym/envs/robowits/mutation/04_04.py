...
@register_task("robowits/04_04-v0")
class PinchCardMut4Env(PinchCardEnv):
    @property
    ...
    def _add_custom_entities(self) -> None:
        ...
        # Code Block: plastic_bag
        _e = self._scene.scene.add_entity(
            gs.morphs.Box(
                pos=(0.55, 0.365, 0.76 + 0.00075),
                euler=(0, 0, 0),
                size=(0.22, 0.16, 0.0015),
                fixed=False,
                collision=True,
            ),
            material=gs.materials.Rigid(rho=200.0, friction=0.05),
            surface=gs.surfaces.Smooth(color=(0.95, 0.95, 0.95), double_sided=True),
        )
        self._entities["plastic_bag"] = {"entity": _e}
        # Code Block: silk_scarf
        _e = self._scene.scene.add_entity(
            gs.morphs.Mesh(
                coacd_options=coacd_options,
                file=get_asset_path("blender_kit/ae9c4515-7b90-4555-a2a2-4c33b5bb274d/obj.glb", pattern_is_dir=False),
                scale=0.6,
                pos=(0.80, 0.42, 0.76 + 0.027 * 0.6),
                euler=(0, 0, 0),
                fixed=False,
                collision=True,
            ),
            material=gs.materials.Rigid(rho=200.0, friction=0.12),
            surface=gs.surfaces.Smooth(double_sided=True),
        )
        self._entities["silk_scarf"] = {"entity": _e}
