...
from gs_gym.common.utils.math_utils import quat_apply
...
class PoseDist(RewardTerm):
    ...
    def _compute(self, pose_a: torch.Tensor, pose_b: torch.Tensor) -> torch.Tensor:  # type: ignore
        # Simple position distance for now - can be extended
        pos_dist = torch.norm(pose_a[:, :3] - pose_b[:, :3], p=2, dim=-1)
        return -pos_dist


class SuccessHeight(RewardTerm):
    """
    Binary reward based on object height threshold.

    Returns 1.0 if object z-position > threshold, else 0.0.

    Args:
        object_pos: Object position tensor of shape (B, 3).
    """

    required_keys = ("object_pos",)

    def __init__(self, threshold: float = 0.1, scale: float = 1.0, name: str | None = None) -> None:
        super().__init__(scale=scale, name=name)
        self.threshold = threshold

    def _compute(self, object_pos: torch.Tensor) -> torch.Tensor:  # type: ignore
        z_height = object_pos[:, 2]  # Get z coordinate
        return (z_height > self.threshold).float()
