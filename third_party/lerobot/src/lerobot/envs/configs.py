...
@EnvConfig.register_subclass("gs_gym")
@dataclass
class GsGymEnv(EnvConfig):
    """Configuration for GS-Gym environments."""

    task: str = "robowits/02-retrieve-cube-v0"
    max_episode_steps: int = 1000
    control_freq: float = 30.0
    headless: bool = True
    fps: int = 30
    video: bool = False
    control_mode: str = "EE_ABS"  # JOINT_ABS (16D), JOINT_DELTA (16D), EE_DELTA (14D), EE_ABS (14D)
    observation_mode: str = "EE"  # EE (14D), JOINT (16D)
    device: str = "cpu"
    eval_dataset_json: str | None = None
    task_ids: list[int] | None = None  # optional index filter for robowits_mutation_XX suite
    # TODO: add more features arguments
    features: dict[str, PolicyFeature] = field(
        default_factory=lambda: {
            "observation.state": PolicyFeature(type=FeatureType.STATE, shape=(14,)),
            "action": PolicyFeature(type=FeatureType.ACTION, shape=(14,)),
            "observation.images.ego": PolicyFeature(type=FeatureType.VISUAL, shape=(3, 480, 848)),
            "observation.images.wrist_right": PolicyFeature(type=FeatureType.VISUAL, shape=(3, 480, 848)),
            "observation.images.wrist_left": PolicyFeature(type=FeatureType.VISUAL, shape=(3, 480, 848)),
        }
    )
    features_map: dict[str, str] = field(
        default_factory=lambda: {
            "observation.state": "observation.state",
            "observation.images.ego": "observation.images.ego",
    ...
    def __post_init__(self):
        action_dim = 16 if self.control_mode in ("JOINT_ABS", "JOINT_DELTA") else 14
        state_dim = 16 if self.observation_mode == "JOINT" else 14
        self.features["observation.state"] = PolicyFeature(type=FeatureType.STATE, shape=(state_dim,))
    ...
    def gym_kwargs(self) -> dict:
        kwargs = {
            ...
            "genesis_backend": self.device,
            "device": self.device,
        }
        if self.eval_dataset_json:
            kwargs["eval_dataset_json"] = self.eval_dataset_json
        if self.task_ids is not None:
            kwargs["task_ids"] = self.task_ids
        return kwargs


@EnvConfig.register_subclass("benchgen")
@dataclass
class BenchgenEnv(EnvConfig):
    """Configuration for benchgen evaluation environments.

    Usage:
        lerobot-eval \\
            --discover_packages_path=gs_gym \\
            --env.type=benchgen \\
            --env.batch_dir=../benchgen/outputs/my_batch \\
            --policy.path=/path/to/checkpoint
    """

    batch_dir: str = ""
    dataset_path: str = ""
    robot_type: str = "marvin"
    episode_length: int = 300
    headless: bool = True
    fps: int = 30
    video: bool = False
    ...
    def gym_kwargs(self) -> dict:
        return {
            ...
            "headless": self.headless,
            "control_mode": self.control_mode,
            "genesis_backend": self.genesis_backend,
        }
