...
def preprocess_observation(observations: dict[str, np.ndarray]) -> dict[str, Tensor]:
    ...
    for extra_key in ("agent_pos_joint", "agent_pos_ee"):
        if extra_key in observations:
            val = torch.from_numpy(observations[extra_key]).float()
            if val.dim() == 1:
                val = val.unsqueeze(0)
            return_observations[f"{OBS_STR}.{extra_key}"] = val
